 ...
 */
public abstract class BaseHipaa277Document extends EDITransaction {

	public static final String DOCUMENT_ID = "277";
	public static final List STANDARDS_IDS = new ArrayList();

	static {
		STANDARDS_IDS.add("004010X093A1,277");
	}
	...
	public void parse(EDIInputStream eis) throws IOException {
		if (log.isDebugEnabled()) log.debug("Starting loop 277");
		...
		while (eis.peekSegment("HL", 3, Loop2000A.TRIGGER_VALUES)) {
			...
			current2000A.setParent((Hipaa277Document)this);
			while (eis.peekSegment("HL", 3, Loop2000B.TRIGGER_VALUES)) {
				...
				while (eis.peekSegment("HL", 3, Loop2000C.TRIGGER_VALUES)) {
					...
					while (eis.peekSegment("HL", 3, Loop2000D.TRIGGER_VALUES)) {
						...
						while (eis.peekSegment("HL", 3, Loop2000E.TRIGGER_VALUES)) {
							Loop2000E current2000E = new Loop2000E();
							current2000E.parse(eis);
							current2000E.setParent(current2000D);
							current2000E.setEndLineNumber(eis.getCurrentSegmentNumber());
							current2000D.addChild(current2000E);
						}
			...
			for (int j = 0; j < current2000A.getChildList().size(); j++) {
				...
				for (int k = 0; k < current2000B.getChildList().size(); k++) {
					...
					for (int l = 0; l < current2000C.getChildList().size(); l++) {
						...
						current2000D.getHl().setHl04((current2000D.getChildList().size() > 0 ? "1" : "0"));
						current2000D.emit(eos);
						for (int m = 0; m < current2000D.getChildList().size(); m++) {
							Loop2000E current2000E = current2000D.getChild(m);
							int current2000EHL = hlidx++;
							current2000E.getHl().setHl01(Integer.toString(current2000EHL));
							current2000E.getHl().setHl02(Integer.toString(current2000DHL));
							current2000E.emit(eos);
						}
					}
				}
			}
	...
	public EDIElement createCopy() {
		Hipaa277Document thisCopy = new Hipaa277Document();
			...
			for (int j = 0; j < current2000A.getChildList().size(); j++) {
				...
				for (int k = 0; k < current2000B.getChildList().size(); k++) {
					...
					for (int l = 0; l < current2000C.getChildList().size(); l++) {
						...
						for (int m = 0; m < current2000D.getChildList().size(); m++) {
							Loop2000E current2000E = current2000D.getChild(m);
							Loop2000E current2000ECopy = (Loop2000E) current2000E.createCopy();
							current2000DCopy.addChild(current2000ECopy);
						}
					}
				}
			}
