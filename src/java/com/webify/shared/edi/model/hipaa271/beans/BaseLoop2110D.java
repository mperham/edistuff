...
public class BaseLoop2110D extends EDILoop {
	...
	// Loops
	private List loop2115D;
	public Loop2115D getLoop2115D(int idx) { 
		if (loop2115D == null || idx >= loop2115D.size()) {
			throw new ArrayIndexOutOfBoundsException("loop2115D");
		}
		return (Loop2115D) loop2115D.get(idx);
	}
	public List getLoop2115D() {
		return loop2115D;
	}
	public void setLoop2115D(List loop) { loop2115D = loop; }

	private LoopUnnamed1 loopUnnamed1;
	public LoopUnnamed1 getLoopUnnamed1() { return loopUnnamed1; }
	public void setLoopUnnamed1(LoopUnnamed1 loop) { loopUnnamed1 = loop; }
	...
	public void parse(EDIInputStream eis) throws IOException {
		...
		while (true) {
			if (eis.peekSegment("III")) {
				if (loop2115D == null) loop2115D = new ArrayList();
				Loop2115D tmp = new Loop2115D();
				tmp.parse(eis);
				loop2115D.add(tmp);
			}
			else if (eis.peekSegment("LS")) {
				if (loopUnnamed1 != null) {
					eis.addError("Only one instance of Loop Unnamed1 is allowed.");
				} else {
					loopUnnamed1 = new LoopUnnamed1();
					loopUnnamed1.parse(eis);
				}
			}
			else {
				break;
		...
		if (loop2115D != null && loop2115D.size() > 10) {
			eis.addError("Loop 2115D has too many instances.  It can repeat only 10 times");
		}
	...
	public void emit(EDIOutputStream eos) throws IOException {
		...
		if (loop2115D != null) {
			int idx = 0, size = loop2115D.size();
			while (idx < size) {
				getLoop2115D(idx++).emit(eos);
			}
		}

		if (loopUnnamed1 != null) {
			loopUnnamed1.emit(eos);
		}
	...
	public EDIElement createCopy() {
		...
		if (loop2115D != null) {
			thisCopy.setLoop2115D(new ArrayList());
			int idx = 0, size = loop2115D.size();
			while (idx < size) {
				thisCopy.getLoop2115D().add(getLoop2115D(idx++).createCopy());
			}
		}

		if (loopUnnamed1 != null) {
			thisCopy.setLoopUnnamed1((LoopUnnamed1)loopUnnamed1.createCopy());
		}
